...
macro_rules! make_attempt {
    ($name:ident, $u:ident, $f:ident, $fstr:expr, $trials:ident, $total_iterations:ident) => {
...
make_attempt!(make_attempt_32, u32, f32, "f32", trials_f32, total_iterations_f32);
make_attempt!(make_attempt_64, u64, f64, "f64", trials_f64, total_iterations_f64);
...
fn main() {
    ...
    for n in 0 .. invocation.float_threads.unwrap_or_else(|| (num_cpus::get() + 1) / 2) {
        ...
        float_handles.push(std::thread::Builder::new()
            .name(format!("f32[{n}]"))
            .spawn(move || {
                worker(make_attempt_32, response_ptr)
            }).unwrap());
    }
    ...
    for n in 0 .. invocation.double_threads.unwrap_or_else(|| (num_cpus::get() + 1) / 2) {
        ...
        double_handles.push(std::thread::Builder::new()
            .name(format!("f64[{n}]"))
            .spawn(move || {
                worker(make_attempt_64, response_ptr)
            }).unwrap());
    }
